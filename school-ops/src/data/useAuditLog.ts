...
// HR/admin must carry an explicit `targetAdminTier == false` filter because
// a Firestore list grant has to hold for EVERY doc the query could match.
// Pre-2026-06-11 entries lack the field and are therefore super_admin-only.
 ...
 * super_admin sees the full log; HR/admin see non-admin-tier entries only;
 * everyone else gets a disabled query.
 */
export function useAuditLog(actor?: Partial<Pick<Actor, "role">> | null, max = 100) {
  const role = actor?.role;
  const scope = role === "super_admin" ? "all" : role === "hr" || role === "admin" ? "nonAdmin" : null;
